...
from mcp_text_editor_launchpad_adapter.models import DeleteTextFileContentsRequest, EditPatch, FileRange
        ...
        try:
            if not os.path.exists(file_path):
                ...
                if not current_file_content:
                    ...
                    suggestion = "patch"
                    hint = "Please use get_text_file_contents tool to get the current content and hashes, then retry with the updated hashes."
        ...
        try:
            ...
            if not self.service.validate_ranges(request.ranges, len(lines)):
                return {
                    request.file_path: {
                        "result": "error",
                        "reason": "Invalid ranges (overlap, out of bounds, or end before start)",
                        "hash": current_hash,
                    }
                }

            # Delete ranges in reverse order to handle line number shifts correctly
            # and validate range_hash for each segment before deletion
            # Create a list of line indices to keep
            lines_to_keep = [True] * len(lines)
            
            for range_to_delete in sorted(request.ranges, key=lambda r: r.start, reverse=True):
                start_idx = range_to_delete.start - 1
                end_idx = range_to_delete.end if range_to_delete.end is not None else total_lines
                
                # Validate range hash before marking for deletion
                if range_to_delete.range_hash is not None:
                    ...
                    if actual_range_hash != range_to_delete.range_hash:
                        return {
                            request.file_path: {
                                "result": "error",
                                "reason": f"Content hash mismatch for range {range_to_delete.start}-{range_to_delete.end}. Expected '{range_to_delete.range_hash}', got '{actual_range_hash}'",
                                "hash": current_hash,
                            }
                        }
            ...
            new_lines = [line for i, line in enumerate(lines) if lines_to_keep[i]]

            # Write new content
            new_content = "".join(new_lines)
            with open(request.file_path, "w", encoding=request.encoding or "utf-8") as f:
                f.write(new_content)
            ...
            logger.error(f"Error deleting contents for {request.file_path}: {str(e)}")
