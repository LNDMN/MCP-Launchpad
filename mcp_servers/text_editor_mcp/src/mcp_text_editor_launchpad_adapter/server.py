...
from mcp_text_editor_launchpad_adapter.handlers import (
...
# Используем __version__ из нашего __init__.py, который будет получать его из version.py или pyproject.toml
from mcp_text_editor_launchpad_adapter import __version__ as launchpad_adapter_version

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("mcp-text-editor-launchpad-adapter")

app: Server = Server(f"mcp-text-editor-launchpad-adapter_v{launchpad_adapter_version}")
...
async def main() -> None:
    ...
    logger.info(f"Starting MCP text editor server v{launchpad_adapter_version}")
    try:
